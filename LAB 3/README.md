 ...
 ## Сгенирировать `a.out`:
```
clang++ main.cpp -lncurses -fsanitize=address
```
## Запустить `a.out`:
```
./a.out
```
  ...
  1) class Character.h - отвечает за основные характеристики "моделей"
  2) class Enemy.h - отвечает за вражеские классы
  3) class Knight.h - герой которым мы управляем
  4) class Zombie - чуваки, которые рандомно перемещаются по карте
  5) Dragon - тоже враг, сильнее чем зомби
  6) Wall - стена - стоит, ничего не делает. Через нее нельзя пройти
  7) GameLoop.h - основная работа проги
  8) Medkit.h - аптечка, дающая одно hp
  9) и другие, лень описывать
